...
import { DocumentInfo, IDocument } from '@core/domain-classes/document-info';
...
export class DocumentService {
  ...
  getDocumentNumbersList():Observable<HttpResponse<IDocument[]> | CommonError>{
  ...
  updateDocumentNumber(data:IDocument){
    const url = 'DocumentNo';
    return this.httpClient.put<IDocument[]>(url,data).pipe(catchError(this.commonHttpErrorService.handleError));
  }

  getNextDocumentNo(type):Observable<IDocument | CommonError> {
    const url = `DocumentNo/GetNextDocumentNo/${type}`;
    return this.httpClient.get<IDocument>(url).pipe(catchError(this.commonHttpErrorService.handleError));
  }
