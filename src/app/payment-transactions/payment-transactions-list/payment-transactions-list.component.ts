  ...
  implements OnInit
{
  ...
  constructor(
    ...
    this.financialYearUUID =
      securityService?.getUserDetail()?.financialYear?.financialYearUUID;
  ...
  loadData(): void {
    this.dataSource = new PaymentTransactionsDataSource(
      this.payementTransactionService
    );
  ...
  getResourceParameter() {
    this.sub$.sink = this.dataSource.responseHeader$.subscribe(
      (c: ResponseHeader) => {
        if (c) {
  ...
  deletePaymentTransaction(pt: IPaymentTransactions): void {
    this.sub$.sink = this.commonDialogService
      .deleteConformationDialog(
        `${this.translate.getValue("COMMON.ARE_YOU_SURE_YOU_WANT_TO_DELETE")} ${
          pt.transactionNo
        }`
      )
      .subscribe((isTrue: boolean) => {
        if (isTrue) {
          this.sub$.sink = this.payementTransactionService
            .deletePaymentTransaction(pt.paymentTransactionUUID)
            .subscribe(() => {
              this.toastr.success(
                this.translate.getValue(
                  "PAYMENT_TRANSACTION_DELETED_SUCCESSFULLY"
                )
              );
