    ...
    if (event.previousContainer === event.container) {
      ...
      this.sub$.sink = this.roleService.updateRoleUsers(this.selectedRole.id, userRolesToSave).subscribe(() => {
        transferArrayItem(event.previousContainer.data,
          ...
          event.previousIndex,
          event.currentIndex);
        this.toastrService.success(`${this.translationService.getValue('RESPONSE_MESSAGE.USER_ADDED_SUCCESSFULLY_TO_ROLE')} ${this.selectedRole.name}`);
      }, () => {
        this.roleUsers.splice(event.previousIndex, 1);
        this.toastrService.error(`${this.translationService.getValue('RESPONSE_MESSAGE.ERROR_WHILE_ADDING_USER_TO_ROLE')} ${this.selectedRole.name}`);
      });
    }
  }
    ...
    if (event.previousContainer === event.container) {
      ...
      this.sub$.sink = this.roleService.updateRoleUsers(this.selectedRole.id, userRolesToSave).subscribe(() => {
        transferArrayItem(event.previousContainer.data,
          ...
          event.previousIndex,
          event.currentIndex);
        this.toastrService.success(`${this.translationService.getValue('RESPONSE_MESSAGE.USER_REMOVED_SUCCESSFULLY_FROM_ROLE')} ${this.selectedRole.name}`);
      }, () => {
        this.toastrService.error(`${this.translationService.getValue('RESPONSE_MESSAGE.ERROR_WHILE_REMOVING_USER_FROM_ROLE')} ${this.selectedRole.name}`);
      });
    }
  }
