...
export class GeneralReportComponent extends BaseComponent implements OnInit {
  ...
  onSearch() {
    if (this.searchForm.valid) {
      ...
      this.sub$.sink = this.stockReportService
        .getGeneralReport(this.stockReportResource)
        .subscribe((res: GeneralStockReport) => {
          this.generalReport = res;
          this.generalStockReportList =
            this.generalReport.stockGeneralReportList;
        });
        this.loader.hide()
    } else {
  ...
  onDownloadReport() {
    ...
    let workSheet = XLSX.utils.sheet_add_json(workBook, generalstockReport, {
      origin: "A2",
      skipHeader: true,
    });
  ...
  generatePdf() {
    let report = this.clonerService.deepClone<GeneralStockReport>(
      this.generalReport
    );
