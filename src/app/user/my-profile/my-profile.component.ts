...
export class MyProfileComponent extends BaseComponent implements OnInit {
  ...
  fileEvent($event) {
    ...
    reader.onload = (_event) => {
      ...
      this.userService.updateProfilePhoto(formData).subscribe((user: User) => {
        this.toastrService.success(this.translationService.getValue('RESPONSE_MESSAGE.PROFILE_PHOTO_UPDATED_SUCCESSFULLY'));
  ...
  removeImage() {
    const formData = new FormData();
    this.userService.updateProfilePhoto(formData).subscribe((user: User) => {
      this.toastrService.success(this.translationService.getValue('RESPONSE_MESSAGE.PROFILE_PHOTO_REMOVED_SUCCESSFULLY'));
      this.imgURL = "";
      this.securityService.updateUserProfile(user);
    });
  }
  ...
  updateProfile() {
    if (this.userForm.valid) {
      ...
      this.sub$.sink = this.userService.updateUserProfile(user)
        .subscribe((user: User) => {
          this.toastrService.success(this.translationService.getValue('RESPONSE_MESSAGE.PROFILE_UPDATED_SUCCESSFULLY'));
          this.securityService.updateUserProfile(user);
        });
    } else {
      this.toastrService.error(this.translationService.getValue('RESPONSE_MESSAGE.PLEASE_ENTER_PROPER_DATA'))
    }
  }
